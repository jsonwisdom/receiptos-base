...
The root page now exposes a deterministic Layer 1 verification workspace. It calls `POST /api/verify` with an `input` field and returns a replayable witness receipt.

Layer 1 rules:

- `authority=false`
- `truth_claim=false`
- observed remains distinct from verified
- receipt identity is derived from SHA-256 over RFC8785 canonical receipt core
- live Base/EAS/ENS/RPC resolution remains `EXTERNAL_PENDING` unless a real external resolver is configured

Run Layer 1 coverage with:
